...
library(emuR)
library(wrassp)
# The chunk arguments below could be simplified to "include = FALSE" but 
# I want to see output in the console 
# the width of the figures is set to 3.7 inches = 0.75*12.5 cm 
# which should be the text width of Lang Sci Press Books
knitr::opts_chunk$set(dev = "cairo_pdf",fig.width = 3.7,fig.asp = 1, 
                      echo = FALSE, message = FALSE, warning = FALSE, 
                      error = FALSE)
```

```{r load_in_acoustic_data}
saxwe_vowel_data <- read_tsv(file.path("..","data","saxwe_vowel_data",
                                       "measurements_from_automatic_boundaries.tsv"))
```

```{r add_vowel_number_in_utterance_column}
saxwe_vowel_data_with_vowel_number <- saxwe_vowel_data %>%
  # each utterance is uniquely identified by the utterance number within the
  # text and the text file name. These are grouped.
  group_by(utterance_num_within_text,text_name) %>% 
  # Then we add a row number starting at 1 for each group. This assumes that
  # the data is ordered in time (which it should be, though we could order it
  # in time in this script).
  mutate(vowel_number_in_utterance = row_number()) %>% 
  ungroup
```

```{r load_in_lexical_tone_info}
# I didn't want it to interpret empty strings as NA so I just put a dummy 
# string in.
lexical_tone_info <- read_tsv(file.path("..","data","saxwe_vowel_data",
                                   "lexical_tone_information.tsv"),
                                   na = "unlikely_dummy_string")
```

```{r combine_the_two_tables}
saxwe_data <- left_join(saxwe_vowel_data_with_vowel_number,lexical_tone_info)
if (nrow(saxwe_data)!=nrow(lexical_tone_info)) stop(
  "the number of rows in the combined table is different to the lexical tone 
  table")
```

```{r add_change_in_semitones_between_syllables_column}
saxwe_semitones <- saxwe_data %>% 
  # combine tone information into one field
  mutate(combo_label = paste(downstep,lexical_tone,ip_boundary,sep="")) %>% 
  mutate(semitone_ref50Hz = f2st(f0,50)) %>%  # add a semitone column
  # check if the utterance is new (this would probably be better handled 
  # with the group_by approach as used above when adding the vowel number)
  mutate(utterance_is_new = 
           utterance_num_within_text != lag(utterance_num_within_text)) %>% 
  mutate(prev_combo_label = lag(combo_label)) %>%
  # create a field that has the tone sequence (from previous to current tone)
  mutate(combo_label_seq=paste(prev_combo_label,"→",combo_label,sep = "")) %>% 
  mutate(change_in_semitones = semitone_ref50Hz - lag(semitone_ref50Hz)) %>% 
  filter(!utterance_is_new) # remove items that span utterances
```

```{r plot_intervals_of_transitions_to_toneless_syllables, fig.cap="Graph showing pitch change for lexical tone transitions to toneless vowels. (The dot plot has a maximum bin width of 0.1 semitones, and the box plot notches indicate 95% confidence intervals for the population median).\\label{fig:transition_to_toneless}"}
saxwe_semitones %>% 
  filter(combo_label_seq=="Ø→Ø"|
         combo_label_seq=="L→Ø"| 
         combo_label_seq=="H→Ø")%>% 
  # Put the groups in the desired order for the x-axis
  mutate(combo_label_seq=
           fct_relevel(combo_label_seq,"Ø→Ø","H→Ø","L→Ø")) %>% 
  ggplot(aes(x=combo_label_seq, y=change_in_semitones)) + 
  # Add in a line for zero for clarity.
  geom_abline(aes(intercept=0, slope=0)) +
  # We remove the box plot whiskers since the following dot plot is a better 
  # representation of outliers
  geom_boxplot(notch=TRUE,outlier.shape = NA,coef = 0,fatten=1)+
  geom_dotplot(binaxis='y', dotsize=0.9, binwidth = 0.1, stackdir = 'center',
               stackratio = 1,colour="white",alpha=0.2) +
  theme_bw()+ theme(legend.position = "none") + xlab("Lexical tone sequence") +
  ylab("Pitch change (semitones)") + 
...
```{r plot_downstep_transition, fig.cap="Graph showing pitch change for lexical tone transitions relevant to downstep. (The dot plot has a maximum bin width of 0.2 semitones, and the box plot notches indicate 95% confidence intervals for the population median).\\label{fig:downstep_transition}"}
saxwe_semitones %>% 
  filter(combo_label_seq=="Ø→!H"|
         combo_label_seq=="H→L"| 
         combo_label_seq=="L→H")%>% 
  # Put the groups in the desired order for the x-axis
  mutate(combo_label_seq=
           fct_relevel(combo_label_seq,"Ø→!H","H→L","L→H")) %>% 
  ggplot(aes(x=combo_label_seq, y=change_in_semitones)) + 
  # Add in a line for zero for clarity.
  geom_abline(aes(intercept=0, slope=0)) +
  # We remove the box plot whiskers since the following dot plot is a better 
  # representation of outliers
  geom_boxplot(notch=TRUE,outlier.shape = NA,coef = 0,fatten=1)+
  geom_dotplot(binaxis='y', dotsize =0.9, stackdir = 'center', binwidth = 0.2,
...
```{r stats_downstep_transition}
high_to_low_distro <- saxwe_semitones %>%  filter(combo_label_seq=="H→L") 
low_to_high_distro <- saxwe_semitones %>%  filter(combo_label_seq=="L→H")
test_result <- wilcox.test(low_to_high_distro$change_in_semitones,
            -high_to_low_distro$change_in_semitones)
p_value <- unlist(test_result)[["p.value"]]
```
...
```{r load-in-acoustic-vowel-data-using-50-percent-reading}
saxwe_vowel_data_manual <- read_tsv(file.path("..","data","saxwe_vowel_data",
                                              "measurements_from_corrected_boundaries.tsv"))
```

```{r add-vowel-number}
manual_data_with_vowel_number <- saxwe_vowel_data_manual %>%
  # each utterance is uniquely identified by the utterance number within the
  # text and the text file name. These are grouped.
  group_by(utterance_num_within_text,text_name) %>% 
  # Then we add a row number starting at 1 for each group. This assumes that
...
```{r combine-with-lexical-tone-info}
manual_data <- left_join(manual_data_with_vowel_number,lexical_tone_info)
if (nrow(manual_data)!=nrow(lexical_tone_info)) stop(
  "the number of rows in the combined table is different to the lexical tone 
  table")
```

```{r add-change-in-semitones-between-syllables-column}
manual_semitones <- manual_data %>% 
  # combine tone information into one field
  mutate(combo_label = paste(downstep,lexical_tone,ip_boundary,sep="")) %>% 
  mutate(semitone_ref50Hz = f2st(f0,50)) %>%  # add a semitone column
  # check if the utterance is new (this would probably be better handled 
...
manual_semitones_collapsed <- manual_semitones %>%
  mutate (collapsed_combo_label_seq = 
    fct_collapse(combo_label_seq, 
                 "T→T before %" = c("H→Htop%", "Ø→Øtop%", "L→Ltop%","H→H%", "Ø→Ø%", "L→L%"), 
                 "T→T crossing %" = c("Htop%→H", "H%→H", "Øtop%→Ø", "Ø%→Ø", "Ltop%→L", "L%→L") ,
                 "T→T" = c("H→H", "Ø→Ø", "L→L")))
```

```{r plot-transitions, fig.cap="Graph showing pitch change for sequences with identical lexical tone as they interact with the utterance-medial IP boundary. This includes the sequence as it approaches the IP boundary and as it crosses the IP boundary.\\label{fig:vowel_sequences_and_medial_boundaries_manual}"}
manual_semitones_collapsed %>% 
  filter(collapsed_combo_label_seq=="T→T before %"|
           collapsed_combo_label_seq=="T→T crossing %"|
           collapsed_combo_label_seq=="T→T")%>% 
  # Put the groups in the desired order for the x-axis
  mutate(collapsed_combo_label_seq=
           fct_relevel(collapsed_combo_label_seq,"T→T","T→T before %","T→T crossing %")) %>% 
  ggplot(aes(x=collapsed_combo_label_seq, y=change_in_semitones)) + 
  # Add in a line for zero for clarity.
  geom_abline(aes(intercept=0, slope=0)) +
  # We remove the box plot whiskers since the following dot plot is a better 
  # representation of outliers
  ...
  geom_dotplot(binaxis='y', dotsize =0.9, stackdir = 'center', binwidth = 0.2,
               colour="white",alpha=0.2) +
  theme_bw()+ theme(legend.position = "none") + xlab("IP boundary and identical tone sequences") +
  ylab("Pitch change (semitones)") +   
  scale_y_continuous(breaks = scales::pretty_breaks(n = 10)) + coord_cartesian(ylim=c(-6,6))
```
...
```{r create-emu-database-from-data, include=FALSE}
# Set up database
path2directory = file.path("..","data","saxwe_acoustic_data")
path2directory
convert_TextGridCollection(path2directory, dbName = "saxwe", 
                           targetDir = tempdir())
path2db = file.path(tempdir(), "saxwe_emuDB")
db_handle = load_emuDB(path2db, verbose = FALSE)
# link utterance tier with word tier
autobuild_linkFromTimes(db_handle, superlevelName = "utterance", 
                        sublevelName = "word", convertSuperlevel = TRUE,
                        newLinkDefType = "ONE_TO_MANY")

# link word tier with phone tier
autobuild_linkFromTimes(db_handle, superlevelName = "word", 
                        sublevelName = "phone", convertSuperlevel = TRUE,
                        newLinkDefType = "ONE_TO_MANY")
```

```{r read-in-hayes-features}
...
features <- read_tsv(file.path("..","data","other_resources",
                          "big_hayes_phone_list_utf8nfc.tsv"))
```

```{r phoneList-function}
# Function `phoneList` modified by Tim Kempton from an original function 
# `phonemeList` (c) Dave Lovell (MIT license) 
# Produces a character vector of phones based on the class specified by the
# input which is a string
# Eg.  phoneList('delayed_release',-1)
phoneList <- function(type,value=NULL)
{
  #Catch bad values 
  if(!is.null(value)&&!(value %in% c(F,T,1,0,-1))){
    stop('Unrecognised \'value\' argument (should be T/F or [-1:1])')}
  if(!(type %in% colnames(features))) {
    stop('\'Type\' argument not in colnames(features)')}
  # Generate character vector 
  if(!is.null(value)&is.numeric(value)) {
    return(as.vector(features$BruceHayes2007[features[[type]]==value]))}
  else {
      stop('integer \'value\' argument required with this phoneme type')}
}
```

```{r define-phonologial-vowel-from-phone-list}
# Get a list of IP vowels
syllabic <- phoneList('syllabic',1)
# Using this list, define an EMU label group called pg_vowel (stands for 
# phonological vowel)
...
```{r find-all-vowels-in-corpus}
all_vowels <- query(db_handle, query = "[phone == pg_vowel]")
```

```{r extract-combo-labels-from-lexical-tone-info}
# although this has been done earlier it didn't exist as a separate variable
tone_label_values <- lexical_tone_info %>% 
  mutate(combo_label = paste(downstep,lexical_tone,ip_boundary,sep="")) %>% 
  select(combo_label) %>%
  rename(tone = combo_label)
```

```{r join-lexical-tone-values-column-to-all-vowels-data}
# add lexical tone to the vowel dataset
# if there is time this should be combined in a safer way e.g. a left_join on 
# a table that has some more data in common
all_vowels_with_tone <- bind_cols(tone_label_values,all_vowels)
if (nrow(all_vowels)!=nrow(tone_label_values)) stop(
  "the number of rows in the combined table is different to the lexical tone 
  table")
seglist_in <- all_vowels_with_tone
```

```{r use-known-speaker-pitch-range-to-constrain-pitch-tracking}
ksvF0Params = formals("ksvF0")
# function ksvF0 seems fairly fragile and can easily crash when called below
# with get_trackdata which seems to more likely when the parameters are changed
# ksvF0Params$gender = "m" there is no need to set both gender and  
# frequency ranges and it will also cause get_trackdata() to crash
ksvF0Params$minF=65
ksvF0Params$maxF=300
```

```{r calculate-pitch-through-duration-of-these-vowels, include=FALSE}
# Calculate pitch
trackdata = get_trackdata(db_handle,
                          seglist = seglist_in,
                          onTheFlyFunctionName = "ksvF0",
                          onTheFlyParams = ksvF0Params)  
# label pitch column more clearly i.e. column heading and make it clear where
# the pitch tracker failed
trackdata_pitch_trajectory <- rename(trackdata,f0=T1) %>% 
  mutate(f0 = na_if(f0,0))
```

```{r filter-for-no-boundary}
filtered_HL_tones <- trackdata_pitch_trajectory %>%
    group_by(tone) %>%
    filter(tone %in% c("H", "L"))
```

```{r vowel-pitch-HL-noboundary-manual, fig.cap="Pitch trajectories of lexical H (top) and L (bottom) vowels wherever IP boundaries are absent. Faint lines show individual trajectories and the solid lines show the median trends. Gray bands show the 95% confidence intervals. \\label{fig:vowel_pitch_HL_noboundary_manual}"}
ggplot(filtered_HL_tones) +
  aes(x = times_norm, y = f0, col = tone, group = tone) +
  #geom_smooth(size=0.5) + # for mean
  stat_quant_line(quantiles =0.5) + # for median and 95% confidence intervals
  geom_line(aes(group=sl_rowIdx),alpha=0.05) + # for individual lines
  labs(x = "Duration of vowel (normalized)", y = "F0 (Hz)") + 
  theme_bw()+ theme(legend.position = "none") +
  coord_cartesian(ylim=c(85,145))
```
...
```{r filter-for-fronted-constituent}
filtered_HL_top_tones <- trackdata_pitch_trajectory %>%
    group_by(tone) %>%
    filter(tone %in% c("Htop%", "Ltop%")) 
```
```{r vowel-pitch-HL-fronted-manual, fig.cap="Pitch trajectories of lexical H (top) and L (bottom) vowels at the boundary for fronted constituents.\\label{fig:vowel_pitch_HL_fronted_manual}"}
ggplot(filtered_HL_top_tones) +
  aes(x = times_norm, y = f0, col = tone, group = tone) +
  #geom_smooth(size=0.5) + # for mean
  stat_quant_line(quantiles =0.5) + # for median and 95% confidence intervals
  geom_line(aes(group=sl_rowIdx),alpha=0.25) + # for individual lines
  labs(x = "Duration of vowel (normalized)", y = "F0 (Hz)") + 
  theme_bw()+ theme(legend.position = "none") +
  coord_cartesian(ylim=c(85,145))
```
...
```{r filter-for-post-clause-constituent} 
filtered_HL_independ_tones <- trackdata_pitch_trajectory %>%
    group_by(tone) %>%
    filter(tone %in% c("H%", "L%"))
```
```{r vowel-pitch-HL-postclause-manual, fig.cap="Pitch trajectories of lexical H (top) and L (bottom) vowels at the boundary for post-clause constituents.\\label{fig:vowel_pitch_HL_postclause_manual}"}
ggplot(filtered_HL_independ_tones) +
  aes(x = times_norm, y = f0, col = tone, group = tone) +
  #geom_smooth(size=0.5) + # for mean
  stat_quant_line(quantiles =0.5) + # for median and 95% confidence intervals
  geom_line(aes(group=sl_rowIdx),alpha=0.25) + # for individual lines
...
```{r filter-for-utterance-final-TBUs} 
filtered_HL_utt_tones <- trackdata_pitch_trajectory %>%
    group_by(tone) %>%
    filter(tone %in% c("Hutt%","Lutt%"))
```
```{r vowel-pitch_HL-uttfinal-manual, fig.cap="Pitch trajectories of lexical H (top) and L (bottom) vowels at the utterance final TBU.\\label{fig:vowel_pitch_HL_uttfinal_manual}"}
ggplot(filtered_HL_utt_tones) +
  aes(x = times_norm, y = f0, col = tone, group = tone) +
  #geom_smooth(size=0.5) + # for mean
  stat_quant_line(quantiles =0.5) + # for median and 95% confidence intervals
  geom_line(aes(group=sl_rowIdx),alpha=0.25) + # for individual lines
